...
C     --- Arguments
	...
	real*8 C,k_B,h_P,m_e,m_H,sigma,a,Pi
...
c$$$  read(*,*)OmegaB,OmegaC,OmegaL
	...
	write(*,*)
        ...
        Tnow=2.728
        Yp=0.245
c$$$	write(*,*)'Enter H_0 (in km/s/Mpc), T_0, Y_p (e.g. 50 2.728 0.24)'
...
c	fixed to match those in the SSS papers, and now correct
	...
	Trad = Tnow * (1.d0+z)
	Hz = HO * dsqrt((1.d0+z)**4/(1.d0+z_eq)*OmegaT
     $        + OmegaT*(1.d0+z)**3 + OmegaK*(1.d0+z)**2 + OmegaL)
...
c	(clunky, but seems to work)
	if (x_H.gt.0.99d0) then	!use Saha rate for Hydrogen
     ...
     $          /(Hz*(1.d0+z)*(1.d0/fu+K*Lambda*n*(1.d0-x)/fu
     $          +K*Rup*n*(1.d0-x)))
	end if
